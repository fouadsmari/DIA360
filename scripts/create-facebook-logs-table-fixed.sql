...
-- Version corrigée pour la structure users existante
...
-- Corrigé pour utiliser users.id au lieu de users.auth_user_id
...
FOR SELECT USING (
  ...
  EXISTS (
    ...
    AND users.poste IN ('Superadmin', 'Direction')
  )
);
...
FOR INSERT WITH CHECK (
  ...
  EXISTS (
    SELECT 1 FROM users 
    WHERE users.id::text = auth.uid()::text
    AND users.poste IN ('Superadmin', 'Direction', 'Responsable')
  )
);
...
-- Corrigé pour utiliser users.id et users.poste
INSERT INTO facebook_logs_config (user_id, retention_days, auto_cleanup)
SELECT DISTINCT id::text, 15, true
FROM users 
WHERE poste IN ('Superadmin', 'Direction', 'Responsable')
