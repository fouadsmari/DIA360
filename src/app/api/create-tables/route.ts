...
export async function POST() {
  try {
    console.log('Tentative de création des tables via Supabase Admin...')
    
    // Essayer de créer les tables une par une en utilisant des requêtes individuelles
    const results = []
    ...
    try {
      ...
      if (testError && testError.code === '42P01') {
        results.push({
          step: 'table_comptes',
          status: 'missing',
          message: 'Table comptes n\'existe pas'
        })
      } else {
        results.push({
          step: 'table_comptes',
      ...
      results.push({
        step: 'table_comptes',
        status: 'error',
        message: `Erreur: ${e instanceof Error ? e.message : 'Inconnue'}`
      })
    }

    // 2. Si les tables n'existent pas, fournir le SQL à exécuter manuellement
    const sqlToExecute = `
...
`
    ...
    return NextResponse.json({
      success: false,
      message: 'Les tables doivent être créées manuellement',
      instructions: 'Copiez et exécutez le SQL ci-dessous dans la console SQL de Supabase',
      sql: sqlToExecute,
      results,
      supabase_console_url: 'https://supabase.com/dashboard/project/zvqvqcbsmvtrcrjeubap/sql/new'
    })
